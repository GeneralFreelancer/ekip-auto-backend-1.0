...
const getSecretes = async (secretesNamesList) => {
    ...
    const secretesList = [];
    for (let i = 0; i < ((_a = secret.Parameters) === null || _a === void 0 ? void 0 : _a.length); i++) {
...
const generateEnvFile = async (secretes) => {
    ...
    secretes.forEach(s => {
        if (s.path.startsWith('/dev/'))
            devEnvFile += `${s.id}=${s.value}\n`;
        if (s.path.startsWith('/prod/'))
            prodEnvFile += `${s.id}=${s.value}\n`;
        if (s.path.startsWith('/stage/'))
            stageEnvFile += `${s.id}=${s.value}\n`;
    });
