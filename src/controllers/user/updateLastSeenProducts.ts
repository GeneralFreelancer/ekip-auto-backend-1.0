...
const updateLastSeenProducts = async (req: Request, res: Response) => {
    const user = req.user
    if (!user) return SendError.UNAUTHORIZED(res, 'Користувача не знайдено')

    const { productId } = req.body

    const updatedUser = await UserService.updateLastSeenProducts(user, productId)
...
export default {
    ...
    handler: controllerWrapper(updateLastSeenProducts),
}
